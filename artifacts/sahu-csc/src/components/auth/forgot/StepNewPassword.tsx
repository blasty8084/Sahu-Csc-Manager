...
import { Lock, Eye, EyeOff, Loader2, CheckCircle2, XCircle, KeyRound } from "lucide-react";
...
/** Step 3 — new password input with per-rule strength indicator and confirm field. */
export function StepNewPassword({ password, confirmPassword, showPassword, showConfirm, submitting, serverError, pwdRulesMet, passwordsMatch, onSubmit, onPasswordChange, onConfirmChange, onToggleShowPassword, onToggleShowConfirm }: StepNewPasswordProps) {
  return (
    <motion.div key="fp-password" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="relative">
          ...
          <Input type={showPassword ? "text" : "password"} placeholder="New password" className="pl-10 pr-10 h-11 text-gray-900 placeholder:text-gray-400 border-gray-200 bg-white focus-visible:ring-2 focus-visible:ring-blue-400" autoFocus autoComplete="new-password" value={password} onChange={(e) => onPasswordChange(e.target.value)} />
          <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" onClick={onToggleShowPassword}>
        ...
        <div className="relative">
          <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />
          <Input type={showConfirm ? "text" : "password"} placeholder="Confirm new password" className="pl-10 pr-10 h-11 text-gray-900 placeholder:text-gray-400 border-gray-200 bg-white focus-visible:ring-2 focus-visible:ring-blue-400" autoComplete="new-password" value={confirmPassword} onChange={(e) => onConfirmChange(e.target.value)} />
          <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" onClick={onToggleShowConfirm}>
