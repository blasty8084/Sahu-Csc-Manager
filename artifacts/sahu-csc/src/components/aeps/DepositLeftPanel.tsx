...
export function DepositLeftPanel({ isWd, txStep, session, isValidAmount, amtNum }: DepositLeftPanelProps) {
  const bg = isWd
    ? "linear-gradient(160deg,#7f1d1d 0%,var(--color-error-deep) 55%,var(--color-error) 100%)"
    : "linear-gradient(160deg,#064e3b 0%,#047857 55%,var(--color-success-light) 100%)";
  ...
  return (
    <div style={{ width: 380, flexShrink: 0, background: bg, display: "flex", flexDirection: "column", padding: "40px 36px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", bottom: -50, left: -50, width: 180, height: 180, borderRadius: "50%", background: "var(--brand-navy-border)", pointerEvents: "none" }} />
      ...
      <div style={{ position: "relative", marginBottom: 24 }}>
        <div style={{ width: 64, height: 64, borderRadius: 20, background: "var(--brand-white-high)", border: "2px solid var(--brand-white-30)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
      ...
      {session && (
        <div style={{ background: "var(--brand-white-mid)", border: "1px solid var(--brand-white-border)", borderRadius: 16, padding: "16px 18px", marginBottom: 12, position: "relative" }}>
          ...
          {isWd && isValidAmount && (
            <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--brand-white-mid)" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
      ...
      {/* Security badge */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, background: "var(--brand-white-low)", border: "1px solid var(--brand-white-high)", borderRadius: 12, padding: "10px 14px", marginTop: "auto", position: "relative" }}>
