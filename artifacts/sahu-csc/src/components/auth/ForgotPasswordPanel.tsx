      ...
      <AnimatePresence mode="wait">
        ...
        {step === "success" && (
          <motion.div key="fp-success" initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className="flex flex-col items-center text-center pt-4">
            ...
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4 font-bold text-white text-lg" style={{ background: "linear-gradient(135deg, var(--brand-navy-650), #0f1a4a)" }}>
              {countdown}
            </div>
            <p className="text-gray-400 text-xs mb-6">Returning to login in {countdown}s</p>
            <Button onClick={onBack} className="w-full h-11 font-bold text-white border-0" style={{ background: "linear-gradient(135deg, var(--brand-navy-650), #0f1a4a)" }}>
              Back to Login
            </Button>
          </motion.div>
        )}
