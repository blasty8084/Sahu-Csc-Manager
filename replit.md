...
- **OTP email delivery fixed (Resend FROM domain)** — `RESEND_FROM` updated to `"SAHU CSC <info@sahucsc.dpdns.org>"` after domain `sahucsc.dpdns.org` was verified in Resend (SPF + DKIM). Previously used `onboarding@resend.dev` (sandbox) which only allowed sending to the Resend account owner. With verified domain, emails deliver to all users including `sahuuttam690@gmail.com`. First-time delivery from a new domain may land in Gmail Spam — mark as "Not spam" once to train Gmail.
