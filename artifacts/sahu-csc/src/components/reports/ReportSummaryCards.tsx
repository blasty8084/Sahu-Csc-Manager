...
export function Sparkline({ data, color }: { data: { v: number }[]; color: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8 }}>
      ...
      <span style={{ fontSize: 10, fontWeight: 700, color: trend >= 0 ? "var(--color-success-light)" : "var(--color-error-std)" }}>
        {trend >= 0 ? "▲" : "▼"} 7d
      </span>
    </div>
  );
    ...
    <div className="bg-white rounded-2xl overflow-hidden flex-1 min-w-0" style={{ boxShadow: "0 2px 12px var(--brand-navy-tint-md)" }}>
      ...
      <div className="p-3">
        <div className="flex items-start justify-between mb-2">
          <p style={{ fontSize: 9, fontWeight: 700, color: "var(--color-slate-400)", textTransform: "uppercase", letterSpacing: "0.06em", lineHeight: 1.2 }}>{label}</p>
...
export function SectionLabel({ label, accentGrad }: { label: string; accentGrad: string }) {
  return (
    <div className="flex items-center gap-2 mb-4">
      ...
      <p style={{ fontSize: 11, fontWeight: 700, color: "var(--color-slate-500)", textTransform: "uppercase", letterSpacing: "0.08em" }}>{label}</p>
    </div>
  );
}
...
export function KpiChip({
  ...
  return (
    <div style={{ flex: 1, padding: "0 20px", borderRight: "1px solid var(--brand-white-low)" }}>
      ...
      {trend && (
        <div style={{ fontSize: 10, fontWeight: 700, color: pos ? "var(--color-success-glow)" : "var(--color-rose-300)" }}>{trend}</div>
      )}
    </div>
  );
}
...
export function EmptyState({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4" style={{ background: "var(--brand-navy-tint-md)" }}>
        <BarChart2 size={28} style={{ color: "var(--color-slate-400)" }} />
      </div>
...
export function DesktopKpiStrip({ activeTab, daily, monthly, aepsReport, breakdown }: DesktopKpiStripProps) {
  ...
  return (
    <div style={{ background: "var(--brand-navy-800)", padding: "14px 28px", display: "flex", flexShrink: 0 }}>
      {chips.map((c, i) => (
        <div
          key={c.label}
          style={{ flex: 1, padding: "0 20px", borderRight: i < chips.length - 1 ? "1px solid var(--brand-white-low)" : "none" }}
        >
          ...
          {c.trend && <div style={{ fontSize: 10, fontWeight: 700, color: c.pos ? "var(--color-success-glow)" : "var(--color-rose-300)" }}>{c.trend}</div>}
        </div>
      ))}
    </div>
  );
