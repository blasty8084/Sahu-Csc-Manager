...
- **Toast system v2 — custom Framer Motion renderer**: `toaster.tsx` replaces Radix UI toast with a pure Framer Motion implementation. `useToast()` hook is unchanged. Variants: `default` (navy), `success` (green `#16a34a`), `destructive` (red), `warning` (amber). Shorthands: `toast.success()`, `toast.error()`, `toast.warning()`, `toast.info()`. Mobile: top-center, slides down. Desktop: bottom-right, slides up (`useIsMobile()` drives the switch at `sm` breakpoint).
- **Toast swipe-to-dismiss**: `drag="x"` on each toast. Dismiss on `|velocity.x| > 400` OR `|offset.x| > 110`. Throw animation via `fmAnimate(x, ±520).then(() => dismiss(id))`. Close button uses `onPointerDown={e.stopPropagation()}` to prevent drag intercepting the tap. Timer paused on `onDragStart`, cleared or restarted on `onDragEnd`.
- **AePS & Udhari receipt tokens**: `receipt_token TEXT` (UUID) on `aeps_transactions` and `udhari_entries`. UUID generated at create time. Public verify pages at `/receipts/verify/aeps/:token` and `/receipts/verify/udhari/:token` (no auth). Public APIs in `aeps.ts` and `receipts.ts`. Route depth (4 segments) prevents the ledger catch-all (3 segments) from shadowing the specific routes.
