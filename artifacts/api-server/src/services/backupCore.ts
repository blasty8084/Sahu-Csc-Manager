...
import { statSync, mkdirSync, existsSync, unlinkSync, readdirSync } from "fs";
...
export async function createBackup(): Promise<BackupRecord> {
  ...
  const [backup] = await db.insert(backupsTable).values({ filename, size: statSync(filepath).size }).returning();
  return fmt(backup);
}
...
export async function getBackupForDownload(id: number) {
  ...
  if (!existsSync(filepath)) throw Object.assign(new Error("Backup file not found on disk"), { status: 404 });
...
export async function restoreBackup(id: number): Promise<{ filename: string }> {
  ...
  if (!existsSync(filepath))
    throw Object.assign(new Error(`Backup file not found on disk: ${backup.filename}`), { status: 404 });
