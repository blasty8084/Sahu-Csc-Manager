...
> **Re-imported and set up on Replit 2026-07-18 (this session)**: `node_modules` missing after import — ran `pnpm install`, pushed DB schema via `pnpm --filter @workspace/db run push-force`, created the `session` table (not managed by Drizzle) via the same SQL in `scripts/post-merge.sh`, built the frontend via `PORT=5000 BASE_PATH=/ pnpm --filter @workspace/sahu-csc run build`. `API Server` (port 8080) and `Start application` (port 5000) workflows confirmed running and serving the app. `Worker Server` skips cleanly — `REDIS_URL` not set, BullMQ falls back to direct/no-op, `CACHE_BACKEND` stays `memory`. **Pending**: `ADMIN_PASSWORD` secret is not set → `Seed Database` cannot run → no accounts exist yet. Add `ADMIN_PASSWORD` (and optionally `OPERATOR_PASSWORD`) in Replit Secrets, then run the `Seed Database` workflow to create the admin account. The `scripts/post-merge.sh` automates all of the above for future merges/imports.
>
