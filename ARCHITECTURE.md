...
| `ALLOW_NON_INDIA=true` shared env var for Replit preview | Replit servers are outside India; the `GeoGate` component calls `/api/geo` and blocks the UI for non-IN IPs. `ALLOW_NON_INDIA=true` bypasses the check in both the Express middleware and the geo endpoint. Leave unset in production to keep geo-restriction active. |
