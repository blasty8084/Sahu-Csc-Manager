...
export function OtpRateLimitPanel({ seconds, onBack }: OtpRateLimitPanelProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-2 text-center">
      <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #fef2f2, var(--color-error-bg))" }}>
      ...
      <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
        <div className="h-full rounded-full" style={{ width: `${(seconds / OTP_RATE_LIMIT) * 100}%`, background: isUrgent ? "var(--color-error-std)" : "var(--brand-orange)", transition: "width 1s linear" }} />
      </div>
      <div>
        <div className="text-4xl font-black tabular-nums tracking-tight" style={{ color: isUrgent ? "var(--color-error-dim)" : "var(--brand-navy-800)" }}>{mm}:{ss}</div>
