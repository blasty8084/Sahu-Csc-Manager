...
export default function ForgotPassword() {
  ...
  // Admin-generated reset link mode
  ...
  // Detect admin-generated reset link (?token=<uuid>&exp=<ms> in URL)
  useEffect(() => {
      ...
      if (exp) {
        ...
        if (!isNaN(expiresAtMs)) {
          const tick = () => {
            ...
            if (secsLeft <= 0) {
              setAdminLinkExpired(true);
              if (adminCountdownRef.current) clearInterval(adminCountdownRef.current);
            }
          };
          tick();
          adminCountdownRef.current = setInterval(tick, 1000);
        }
      }
    }
    return () => {
                ...
                {adminResetMode && adminLinkExpired && (
                  <div className="mb-4 rounded-xl border border-red-300 bg-red-50 px-4 py-3 flex items-start gap-2.5 text-xs text-red-800">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-red-700">This link is no longer valid. Please ask your administrator to generate a new one.</p>
                    </div>
                  </div>
                )}
                ...
                {adminResetMode && !adminLinkExpired && (
                  <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 flex items-start gap-2.5 text-xs text-blue-800">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="mt-0.5 text-blue-700">This link was generated by your administrator and can only be used once.</p>
                          ...
                          <div className="flex-1 h-1.5 rounded-full overflow-hidden bg-blue-200">
                            <div
                              ...
                              style={{
                                ...
                                background: adminLinkSecondsLeft <= 60 ? "#ef4444" : adminLinkSecondsLeft <= 120 ? "#f97316" : "#2563eb",
                              }}
                            />
                          </div>
                          <span
                            className="font-black tabular-nums shrink-0 text-sm"
                            style={{ color: adminLinkSecondsLeft <= 60 ? "#dc2626" : adminLinkSecondsLeft <= 120 ? "#ea580c" : "#1d4ed8" }}
                          >
                            {String(Math.floor(adminLinkSecondsLeft / 60)).padStart(2, "0")}:{String(adminLinkSecondsLeft % 60).padStart(2, "0")}
                          </span>
