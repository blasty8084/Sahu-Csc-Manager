...
function StatCard({ icon: Icon, label, value, accent }: {
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-zinc-800 border border-slate-100 dark:border-zinc-700 p-4 flex items-center gap-3"
      ...
      <div>
        <p className="text-2xl font-black text-slate-800 dark:text-zinc-100 leading-none">{value}</p>
  ...
  if (statsLoading) {
    return (
      <div className="grid grid-cols-2 gap-3">
        {[0, 1].map((i) => <div key={i} className="rounded-2xl bg-white dark:bg-zinc-800 h-16 animate-pulse" />)}
      </div>
    );
  }
