...
> **Re-imported and set up on Replit 2026-07-18**: `node_modules` was missing after import. Ran `pnpm install`, pushed DB schema via `cd lib/db && pnpm run push-force` (tables created fresh against existing `DATABASE_URL`), built the frontend via `PORT=5000 BASE_PATH=/ pnpm --filter @workspace/sahu-csc run build`, then restarted `API Server` and `Start application` workflows — both confirmed running. `Worker Server` skips cleanly (no `REDIS_URL` set). `REDIS_URL`/Upstash not present, so BullMQ jobs fall back to direct/no-op and `CACHE_BACKEND` remains `memory`. **Pending**: `ADMIN_PASSWORD` secret is not set, so the `Seed Database` workflow cannot run — add it in Replit Secrets and re-run `Seed Database` to create the admin account. Note: the platform auto-registers extra artifact workflows (`artifacts/sahu-csc: web`, `artifacts/api-server: API Server`) on every import — these are unrelated to this project's real setup and can be ignored; use the named `API Server`, `Start application`, and `Worker Server` workflows only.
>
