...
| **Workflow cleanup** | `Seed Database` now pushes schema before seeding (self-contained, safe to run standalone). `Worker Server` uses clean `exit 0` instead of printing a skip message. `Start application` fixed to use `run dev` instead of the broken `run serve`. `Project` runButton updated to launch `artifacts/sahu-csc: web` (was `Start application`). |
