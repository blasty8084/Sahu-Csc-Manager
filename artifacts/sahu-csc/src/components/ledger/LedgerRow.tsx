...
export function TableTabsHeader({
  ...
  return (
    <div style={{ display: "flex", borderBottom: "1px solid var(--color-slate-200)", flexShrink: 0, paddingLeft: 4 }}>
      {(["transactions", "receipts"] as const).map(tab => (
        <button key={tab} onClick={() => setActiveTab(tab)}
          style={{ display: "flex", alignItems: "center", gap: 6, padding: "14px 20px", borderBottom: `2px solid ${activeTab === tab ? "var(--brand-navy-800)" : "transparent"}`, color: activeTab === tab ? "var(--brand-navy-800)" : "var(--color-slate-400)", fontWeight: activeTab === tab ? 700 : 600, fontSize: 13, background: "transparent", border: "none", cursor: "pointer", transition: "all 0.15s" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, paddingRight: 16 }}>
        <p style={{ fontSize: 11, color: "var(--color-slate-400)" }}>
          {activeTab === "transactions"
            ? `${data?.total ?? 0} entries · Page ${page} of ${Math.max(totalPages, 1)}`
            : `${receiptEntries.length} receipt${receiptEntries.length !== 1 ? "s" : ""}`}
        </p>
        {hasFilters && <span style={{ fontSize: 10, fontWeight: 700, color: "var(--brand-orange)", background: "var(--brand-orange-tint-xs)", border: "1px solid var(--brand-orange-tint-md)", borderRadius: 20, padding: "3px 10px" }}>Filtered</span>}
        {isOffline && <span style={{ fontSize: 10, fontWeight: 700, color: "var(--color-warning)", background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.25)", borderRadius: 20, padding: "3px 8px", display: "flex", alignItems: "center", gap: 3 }}><WifiOff size={9} />Offline</span>}
        {bgSyncCount > 0 && <span style={{ fontSize: 10, fontWeight: 700, color: "var(--color-violet)", background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.2)", borderRadius: 20, padding: "3px 8px", display: "flex", alignItems: "center", gap: 3 }}><Clock size={9} />{bgSyncCount} retrying</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <>
      {pendingEntries.length > 0 && (
        <div style={{ background: "rgba(251,191,36,0.07)", borderBottom: "1px solid var(--color-warning-tint)", padding: "10px 22px", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          <Clock size={13} style={{ color: "var(--color-warning)", flexShrink: 0 }} />
          <p style={{ fontSize: 12, fontWeight: 600, color: "var(--color-warning-dark)" }}>
      ...
      {bgSyncCount > 0 && (
        <div style={{ background: "rgba(124,58,237,0.06)", borderBottom: "1px solid rgba(124,58,237,0.16)", padding: "10px 22px", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          <Clock size={13} style={{ color: "var(--color-violet)", flexShrink: 0 }} />
...
export function DesktopLedgerRow({
  ...
  return (
    <tr data-testid={`row-ledger-${entry.id}`}
      style={{ borderBottom: "1px solid var(--color-slate-100)", transition: "background 0.1s" }}
      onMouseEnter={e => (e.currentTarget.style.background = "rgba(11,44,96,0.018)")}
      onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
      <td style={{ padding: "14px 14px", color: "var(--color-gray-300)", fontSize: 11, fontWeight: 700 }}>{rowNum}</td>
      <td style={{ padding: "14px 14px", whiteSpace: "nowrap" }}>
        <code style={{ fontFamily: "monospace", fontSize: 11, fontWeight: 800, color: "var(--brand-orange)", background: "var(--brand-orange-tint-07)", padding: "3px 8px", borderRadius: 6, border: "1px solid var(--brand-orange-tint-sm)" }}>
          {entry.receiptNumber ?? `CSC-${new Date(entry.createdAt).getFullYear()}-${String(entry.id).padStart(4, "0")}`}
        </code>
      </td>
      <td style={{ padding: "14px 14px", fontSize: 12, fontWeight: 600, color: "var(--color-slate-500)", whiteSpace: "nowrap" }}>{entry.date}</td>
      <td style={{ padding: "14px 14px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: "var(--brand-navy-800)", maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>{entry.customerName}</span>
        </div>
      </td>
      <td style={{ padding: "14px 14px" }}>
      ...
      <td style={{ padding: "14px 14px", textAlign: "right", fontWeight: 800, fontSize: 13, whiteSpace: "nowrap" }}>
        {entry.credit > 0 ? (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            ...
            <span style={{ color: "var(--color-success)" }}>+₹{entry.credit.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
          </span>
        ) : <span style={{ color: "var(--color-slate-200)" }}>—</span>}
      </td>
      <td style={{ padding: "14px 14px", textAlign: "right", fontWeight: 800, fontSize: 13, whiteSpace: "nowrap" }}>
        {entry.debit > 0 ? (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--color-error)", display: "inline-block", flexShrink: 0 }} />
            <span style={{ color: "var(--color-error)" }}>−₹{entry.debit.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
          </span>
        ) : <span style={{ color: "var(--color-slate-200)" }}>—</span>}
      </td>
      <td style={{ padding: "14px 14px", textAlign: "right", fontWeight: 900, fontSize: 13, color: balanceNum < 0 ? "var(--color-error)" : "var(--brand-navy-800)", whiteSpace: "nowrap" }}>
        ₹{balanceNum.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
      </td>
      <td style={{ padding: "14px 14px", fontSize: 11, color: "var(--color-slate-400)", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.description || "—"}</td>
...
export function MobileLedgerCard({
  ...
  const iconBg = isCredit ? "var(--color-success-bg)" : "var(--color-error-bg)";
  return (
    <div data-testid={`row-ledger-${entry.id}`}
      style={{ background: "#fff", borderRadius: 14, marginBottom: 8, overflow: "hidden", boxShadow: "0 1px 8px var(--brand-navy-tint-md)", display: "flex", border: "1px solid var(--color-slate-100)" }}>
      ...
      <div style={{ flex: 1, padding: "11px 12px", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: "var(--brand-navy-800)", lineHeight: 1.2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.customerName}</p>
          <p style={{ fontSize: 11, color: "var(--color-slate-400)", marginTop: 2 }}>{entry.serviceType}</p>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <p style={{ fontSize: 15, fontWeight: 900, color: ec, lineHeight: 1 }}>{isCredit ? "+" : "−"}₹{amt.toLocaleString("en-IN")}</p>
          <p style={{ fontSize: 10, color: "var(--color-slate-400)", marginTop: 3 }}>Bal ₹{Number(entry.balance).toLocaleString("en-IN")}</p>
