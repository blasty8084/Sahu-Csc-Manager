...
function LoginFormContent({ form, onSubmit, showPassword, setShowPassword, rememberMe, setRememberMe, onForgotPassword }: LoginFormContentProps) {
...
export default function Login() {
  ...
  const onSubmit = async (values: LoginFormValues) => {
    ...
    try {
      ...
      if (err?.locked) {
        ...
        toast({ variant: "destructive", title: "Login failed", description: `${err.message ?? "Invalid credentials."} (${err.attemptsLeft} attempt${err.attemptsLeft !== 1 ? "s" : ""} remaining)` });
      } else {
  ...
  return isMobile
    ? <MobileLogin {...formProps} onForgotPassword={() => {}} />
    : <DesktopLogin {...formProps} onForgotPassword={() => {}} />;
}
