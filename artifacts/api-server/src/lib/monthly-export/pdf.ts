...
export function generateReceiptPdf(entry: ReceiptEntry, biz: BizInfo): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    y = stripeY + 4;
    const blockX = 32, blockW = W - 64, blockY = y + 22, blockH = 72;

    doc.roundedRect(blockX, blockY, blockW, blockH, 8).fillAndStroke(`${amountColor}18`, `${amountColor}35`);
    ...
    const circX = blockX + blockW - 40, circY = blockY + blockH / 2;
    ...
    const rows: Array<{ label: string; value: string }> = [
      ...
      { label: "Issued At", value: new Date(entry.createdAt).toLocaleString("en-IN", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }) },
      ...(entry.createdByName ? [{ label: "Operator", value: entry.createdByName }] : []),
      ...(entry.description ? [{ label: "Note", value: entry.description }] : []),
    ];

    const rowX = 32, rowW = W - 64, rowH = 30;
    rows.forEach((row, i) => {
      const rowY = y + i * rowH;
      if (i < rows.length - 1) doc.moveTo(rowX, rowY + rowH).lineTo(rowX + rowW, rowY + rowH).stroke("#f1f5f9");
      doc.font("Helvetica").fontSize(9).fillColor("#94a3b8").text(row.label, rowX, rowY + 9);
      doc.font("Helvetica-Bold").fontSize(10).fillColor(NAVY).text(row.value, rowX + rowW / 2, rowY + 9, { width: rowW / 2, align: "right" });
    });
    ...
    const footerH = 44, footerY = 841.89 - footerH;
