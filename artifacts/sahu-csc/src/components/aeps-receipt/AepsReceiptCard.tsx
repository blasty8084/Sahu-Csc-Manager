...
import { MapPin, Phone, Globe, Fingerprint, ShieldCheck, CheckCircle2 } from "lucide-react";
...
/** Printable AePS receipt body. */
export function AepsReceiptCard({ tx, containerRef, businessName, businessAddress, businessMobile, businessWebsite }: Props) {
  ...
  const headerGrad    = isWithdrawal ? "linear-gradient(135deg, #7f1d1d 0%, #be1240 55%, #6b1228 100%)" : "linear-gradient(135deg, #064e3b 0%, #059669 55%, #044032 100%)";
  const stripeGrad    = isWithdrawal ? "linear-gradient(90deg, #e11d48, #f43f5e 50%, #e11d48)" : "linear-gradient(90deg, #059669, #34d399 50%, #059669)";
  ...
  const qrValue       = verifyUrl ?? [`SAHU CSC – AePS Receipt`, `Receipt: ${receiptNumber}`, `Type: ${txLabel}`, `Customer: ${tx.customerName}`, `Amount: ₹${tx.amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`, `Date: ${formattedDate}`].join("\n");
    ...
    <div ref={containerRef} style={{ background: "#fff" }}>
      ...
      <div style={{ padding: "14px 22px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px dashed #e2e8f0" }}>
        ...
        <div style={{ textAlign: "right" }}>
          {hasToken
            ? <div style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 20, padding: "3px 9px" }}><CheckCircle2 size={10} color="#22c55e" strokeWidth={2.5} /><span style={{ fontSize: 9, fontWeight: 700, color: "#22c55e", letterSpacing: "0.06em" }}>VERIFIED</span></div>
      ...
      <div style={{ padding: "18px 22px 14px", textAlign: "center", position: "relative" }}>
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: 0.025, pointerEvents: "none" }}><ShieldCheck size={110} color={amountColor} strokeWidth={1} /></div>
      ...
      <div style={{ margin: "0 16px 14px", background: "#f8fafc", borderRadius: 16, padding: "12px 16px", border: "1px solid #f1f5f9" }}>
        {[{ label: "Customer", value: tx.customerName }, { label: "Transaction", value: txLabel }, { label: "Date", value: formattedDate }, { label: "Issued", value: issuedAt }, ...(tx.balance !== undefined ? [{ label: "Balance After", value: `₹${tx.balance.toLocaleString("en-IN", { minimumFractionDigits: 2 })}` }] : []), ...(tx.description ? [{ label: "Note", value: tx.description }] : [])].map((row, i, arr) => (
      ...
      <div style={{ background: "#f8fafc", borderTop: "1px solid #f1f5f9", padding: "10px 22px", textAlign: "center" }}>
        <p style={{ fontSize: 10, color: "#94a3b8", fontWeight: 600 }}>AePS transaction receipt · Aadhaar-Enabled Payment System</p>
      </div>
    </div>
  );
}
