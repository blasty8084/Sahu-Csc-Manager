...
| **`PENDING` users cannot log in** | `POST /api/auth/register` always creates users with `status = 'PENDING'`. The login handler checks status before password verification — `PENDING` returns 403. An admin must approve via Users → Pending tab before the account is usable. |
| **Registration toggle is runtime, not env-based** | `registration_open` is stored in the `settings` table and cached in-process (60 s TTL). It is toggled by admin from the Settings UI — not set as an env variable or deploy config. On server restart the cache is empty; the first request re-reads from DB. |
| **Pending count cache invalidates immediately** | `admin:pending_approvals` in-process cache is deleted after every approve/reject. The pending count badge shows the stale (cached) value until the next 30-s poll, then updates. Do not expect real-time accuracy — the cache is a performance optimisation, not the source of truth. |
| **New pages must use `React.lazy()`** | All pages except `Login`, `NotFound`, and `Offline` are lazy-loaded. Adding a new page requires `const Foo = lazy(() => import("@/pages/foo"))` in `App.tsx` — a static import would add it to the initial bundle and undo the code-splitting optimisation. |
| **Vendor chunks in vite build config** | `vite.config.ts` `rollupOptions.output.manualChunks` splits React, TanStack Query, Framer Motion, Wouter, Recharts, and Lucide into separate cache entries. Do not remove — they allow browser to cache vendor assets independently of app code. |
