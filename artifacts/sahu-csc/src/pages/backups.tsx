...
import { BackupHistorySkeleton, BackupScheduleSkeleton } from "@/components/skeletons";
...
export default function Backups() {
  ...
  const totalSize = useMemo(() => (backups ?? []).reduce((s: number, b: any) => s + (b.size ?? 0), 0), [backups]);
                ...
                <AreaChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number, name: string) => [
                      `${value} KB`,
                      name === "sizeKB" ? "Snapshot size" : "Cumulative storage",
                    ]}
                  ...
                  <Area
                    ...
                    strokeWidth={2}
                    fill="url(#gradSize)"
                    dot={{ r: 3, fill: "#0b2c60", strokeWidth: 0 }}
                    activeDot={{ r: 5, fill: "#0b2c60" }}
                  />
                  <Area
                    type="monotone"
                    dataKey="totalKB"
