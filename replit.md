...
- **receipt-modal split** — `components/receipt-modal.tsx` reduced from 443 → 84 lines by extracting `receipt/receiptTypes.ts` (29 ln — types), `receipt/ReceiptCard.tsx` (142 ln — printable body), `receipt/ReceiptActionBar.tsx` (34 ln — action buttons), `receipt/useReceiptActions.ts` (136 ln — PDF/print/share hook). TypeScript clean.
- **ForgotPasswordPanel split** — `components/auth/ForgotPasswordPanel.tsx` reduced from 404 → 207 lines by extracting 4 step sub-components into `auth/forgot/`: `ForgotIdentifierStep` (56 ln), `ForgotOtpStep` (83 ln), `ForgotPasswordStep` (87 ln), `ForgotSuccessStep` (32 ln). TypeScript clean.
- **TwoFactorStep split** — `components/auth/TwoFactorStep.tsx` reduced from 395 → 193 lines by extracting 4 sub-components into `auth/twofa/`: `TwoFactorMethodPicker` (64 ln), `TotpEnrollmentCard` (56 ln), `TwoFactorBackupCodesScreen` (41 ln), `TwoFactorCodeEntry` (120 ln). TypeScript clean.
- **LoginCredentialsStep split** — `components/auth/LoginCredentialsStep.tsx` reduced from 511 → 89 lines by extracting `LoginRejectedPanel` (129 ln — declined card + appeal buttons), `LoginPendingPanel` (49 ln — awaiting approval), `LoginLockoutPanel` (66 ln — countdown + progress bar), `LoginFormFields` (174 ln — full credential form with attempt counter). Thin orchestrator calls `useLockoutCountdown` once, passes `remaining`/`display`/`progress` to lockout panel. TypeScript clean.
- **DesktopReports split** — `components/reports/DesktopReports.tsx` reduced from 463 → 101 lines by extracting `hooks/useReportPrint.ts` (178 ln — full print-window HTML builder as a hook), `hooks/useReportKpis.ts` (59 ln — KPI chip array per active tab), `components/reports/DesktopReportsContent.tsx` (144 ln — all four scrollable tab panels). TypeScript clean.
- **LedgerTable split** — `components/ledger/LedgerTable.tsx` reduced from 606 → 25 lines (barrel). Four new focused sub-components: `LedgerEmptyState.tsx` (32 ln — no-transactions empty state illustration), `LedgerRowActions.tsx` (42 ln — Receipt/Edit/Delete button cluster, `size="sm|md"` for mobile/desktop), `LedgerRow.tsx` (123 ln — `LedgerDesktopRow` `<tr>` + `LedgerMobileRow` card entry, both using `LedgerRowActions`), `LedgerPagination.tsx` (104 ln — absorbs `TableFooterPagination` + `MobilePagination`). Six existing named exports moved to `ledger/table/`: `TableTabsHeader` (37 ln), `PendingSyncBanners` (24 ln), `DesktopReceiptsPanel` (85 ln), `DesktopTransactionsTable` (156 ln — uses `LedgerDesktopRow` + `LedgerEmptyState`), `MobileReceiptsList` (73 ln), `MobileTransactionsList` (47 ln — uses `LedgerMobileRow`). All import sites (`pages/ledger.tsx`, `pages/aeps/all-tx/AllTxTable.tsx`) unchanged. TypeScript clean.
