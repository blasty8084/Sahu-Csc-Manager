...
export function DesktopReceiptExpandedPreview({
  ...
  return (
    <div className="bg-white dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-700 shadow-sm overflow-hidden">
      <div className="bg-slate-700 dark:bg-zinc-700 px-4 py-2.5 flex items-center gap-2">
        <Eye size={13} className="text-white/70" />
        <span className="text-xs font-semibold text-white">Receipt Preview</span>
      </div>
      <div className="p-4">
        <div className="border border-dashed border-slate-200 dark:border-zinc-600 rounded-lg p-3 bg-slate-50/50 dark:bg-zinc-900/60 text-center">
          ...
          <p className="text-[9px] text-slate-400 dark:text-zinc-500 mb-2">Common Service Center, Odisha</p>
          <div className="border-t border-dashed border-slate-200 dark:border-zinc-600 pt-2 text-left space-y-1">
            {[["Receipt #", e.receiptNumber], ["Date", fmtDate(e.date)], ["Customer", e.customerName], ["Service", e.serviceType]].map(([k, v]) => (
              <div key={k} className="flex justify-between text-[10px]">
                ...
                <span className="font-semibold text-slate-800 dark:text-zinc-100 text-right max-w-[110px] truncate">{v}</span>
              </div>
            ))}
          </div>
          <div className="border-t border-dashed border-slate-200 dark:border-zinc-600 mt-2 pt-2 flex justify-between items-center">
            <span className="text-[10px] font-bold text-slate-800 dark:text-zinc-100">Total Paid</span>
          ...
          <div className="mt-2.5 flex justify-center">
            <div className="w-14 h-14 bg-slate-100 dark:bg-zinc-700 border border-slate-200 dark:border-zinc-600 rounded flex items-center justify-center">
              <QrCode size={28} className="text-slate-400 dark:text-zinc-500" />
            </div>
          </div>
          <p className="text-[9px] text-slate-400 dark:text-zinc-500 mt-1">Scan to verify online</p>
          <div className="mt-2 flex items-center gap-1 justify-center">
            <div className="h-px flex-1 border-t border-dashed border-slate-200 dark:border-zinc-600" />
            <span className="text-[9px] text-slate-400 dark:text-zinc-500">Thank you</span>
            <div className="h-px flex-1 border-t border-dashed border-slate-200 dark:border-zinc-600" />
          </div>
        </div>
        <div className="mt-3 grid grid-cols-3 gap-1.5">
          {([
            ...
            <button key={label}
              ...
              className="flex flex-col items-center gap-1 py-2 rounded-lg bg-slate-50 dark:bg-zinc-700 hover:bg-slate-100 dark:hover:bg-zinc-600 transition-colors text-slate-600 dark:text-zinc-300 disabled:opacity-40">
