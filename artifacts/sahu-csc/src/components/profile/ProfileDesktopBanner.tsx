...
export function ProfileDesktopBanner({
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(135deg, var(--brand-navy-800) 0%, #0d3270 55%, var(--brand-navy-700) 100%)" }}
    >
        ...
        <div className="flex items-center gap-5">
          <div className="relative shrink-0">
            <Avatar className="h-20 w-20 border-2 border-white/20">
              ...
              <AvatarFallback className="text-3xl font-bold bg-[var(--brand-orange)] text-white">{initials}</AvatarFallback>
            </Avatar>
            <button type="button" onClick={onShowPicker}
              className="absolute -bottom-1 -right-1 rounded-full bg-[var(--brand-orange)] text-white p-1.5 shadow-md hover:bg-orange-600 transition-colors">
              <Camera size={12} />
            </button>
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              <span className="px-2.5 py-0.5 rounded-full text-xs font-bold capitalize"
                style={{ background: "var(--brand-orange-tint-18)", color: "var(--brand-orange-400)", border: "1px solid var(--brand-orange-border)" }}>
                {profile?.role}
              </span>
        ...
        <div className="flex items-center gap-3 shrink-0">
          {[
            ...
            <div key={k.label} className="text-center px-5 py-3 rounded-xl border border-white/12"
              style={{ background: "var(--brand-white-glass)" }}>
              <div className="flex items-center justify-center gap-1 mb-1" style={{ color: "var(--brand-orange-400)" }}>{k.icon}</div>
