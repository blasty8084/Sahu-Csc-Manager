...
export function UdhariAddEntryForm({ customerId, mode, existing, open, onClose, customer }: Props) {
  ...
  const headerGrad  = isGave ? "linear-gradient(145deg,var(--color-orange-900),var(--brand-orange-600))" : "linear-gradient(145deg,#064e3b,var(--color-success))";
  const accentBg    = isGave ? "rgba(234,88,12,0.08)" : "var(--color-success-bg)";
  const accentBorder = isGave ? "rgba(234,88,12,0.2)" : "rgba(5,150,105,0.2)";
  const amtGrad     = isGave ? "linear-gradient(135deg,var(--color-orange-900),var(--brand-orange-600))" : "linear-gradient(135deg,#064e3b,var(--color-success))";
  ...
  const previewColor   = previewBalance > 0 ? "var(--brand-orange-600)" : previewBalance < 0 ? "var(--color-success)" : "var(--color-slate-500)";

  const TypeToggle = () => !isEdit ? (
    <div style={{ background: "var(--color-slate-100)", borderRadius: 14, padding: 4, display: "flex", gap: 4 }}>
      {(["gave", "got"] as const).map(typ => (
        <button key={typ} type="button" onClick={() => setForm(p => ({ ...p, type: typ }))}
          style={{ flex: 1, height: 42, borderRadius: 11, border: "none", cursor: "pointer", fontWeight: 800, fontSize: 13, background: form.type === typ ? (typ === "gave" ? "var(--brand-orange-600)" : "var(--color-success)") : "transparent", color: form.type === typ ? "#fff" : "var(--color-slate-400)", transition: "all 0.15s", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, boxShadow: form.type === typ ? `0 2px 10px ${typ === "gave" ? "rgba(234,88,12,0.35)" : "color-mix(in srgb, var(--color-success) 35%, transparent)"}` : "none" }}>
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="w-[calc(100vw-2rem)] max-w-sm rounded-2xl p-0 overflow-hidden gap-0">
        ...
        <div style={{ background: headerGrad, margin: "12px 16px 0", borderRadius: 18, padding: "14px 16px", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", top: -16, right: -16, width: 70, height: 70, borderRadius: "50%", background: "var(--brand-white-low)", pointerEvents: "none" }} />
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 38, height: 38, borderRadius: 11, background: "var(--brand-white-mid)", border: "1.5px solid var(--brand-white-25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: 8, background: "var(--brand-white-mid)", border: "1px solid var(--brand-white-25)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", outline: "none" }}>
              <X size={13} color="#fff" />
            </button>
          </div>
          {customer && (
            <div style={{ background: "var(--brand-white-mid)", borderRadius: 10, padding: "7px 12px", display: "flex", alignItems: "center", gap: 8, marginTop: 10 }}>
              <div style={{ width: 24, height: 24, borderRadius: "50%", background: "var(--brand-white-border)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              <div style={{ background: "var(--brand-white-mid)", borderRadius: 6, padding: "3px 8px", flexShrink: 0 }}>
        ...
        <div className="px-4 pt-3 pb-5 space-y-3">
          ...
          <div style={{ background: accentBg, border: `2px solid ${accentBorder}`, borderRadius: 16, padding: "13px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "var(--color-slate-400)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 2 }}>{isGave ? t("udhari.customer.amount_gave") : t("udhari.customer.amount_got")}</p>
          ...
          <div style={{ position: "relative" }}>
            ...
            <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))}
              style={{ width: "100%", height: 44, paddingLeft: 36, paddingRight: 12, borderRadius: 12, border: "1.5px solid var(--color-slate-200)", background: "var(--surface-card-near-white)", fontSize: 14, color: "var(--brand-navy-800)", outline: "none", boxSizing: "border-box", fontWeight: 600 }} />
          </div>
          <div style={{ position: "relative" }}>
            ...
            <textarea value={form.note} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} rows={2} placeholder="Add a note (optional)"
              style={{ width: "100%", paddingLeft: 36, paddingRight: 12, paddingTop: 12, paddingBottom: 12, borderRadius: 12, border: "1.5px solid var(--color-slate-200)", background: "var(--surface-card-near-white)", fontSize: 13, color: "var(--brand-navy-800)", outline: "none", resize: "none", boxSizing: "border-box", fontFamily: "inherit" }} />
          </div>
          {customer && entryAmt > 0 && (
            <div style={{ background: "var(--color-slate-50)", borderRadius: 12, padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", border: "1px solid var(--color-slate-200)" }}>
              <p style={{ fontSize: 12, fontWeight: 600, color: "var(--color-slate-500)" }}>New balance after this entry</p>
              <p style={{ fontSize: 14, fontWeight: 900, color: previewColor }}>{previewLabel}</p>
            </div>
          )}
          <div style={{ display: "flex", gap: 10 }}>
            <button type="button" onClick={onClose}
              style={{ flex: 1, height: 52, borderRadius: 16, border: "1.5px solid var(--color-slate-200)", background: "var(--color-slate-50)", cursor: "pointer", fontWeight: 700, fontSize: 14, color: "var(--color-slate-500)" }}>
              {t("common.cancel")}
            </button>
            <button type="button" onClick={handleSave} disabled={create.isPending || update.isPending}
              style={{ flex: 2, height: 52, borderRadius: 16, border: "none", cursor: "pointer", background: isGave ? "linear-gradient(135deg,var(--color-orange-900),var(--brand-orange-600))" : "linear-gradient(135deg,#064e3b,var(--color-success))", color: "#fff", fontSize: 16, fontWeight: 900, letterSpacing: "0.02em", boxShadow: `0 6px 20px ${accentColor}40`, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: (create.isPending || update.isPending) ? 0.7 : 1 }}>
    ...
    <>
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex" }}>
        ...
        <div style={{ width: 380, flexShrink: 0, background: headerGrad, display: "flex", flexDirection: "column", padding: "40px 36px", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", top: -70, right: -70, width: 240, height: 240, borderRadius: "50%", background: "var(--brand-white-glass)", pointerEvents: "none" }} />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 40, position: "relative" }}>
            <div style={{ width: 36, height: 36, borderRadius: 11, background: "var(--brand-white-border)", border: "1.5px solid var(--brand-white-30)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontWeight: 900, fontSize: 13, color: "#fff" }}>SC</span>
            </div>
            <span style={{ color: "#fff", fontWeight: 900, fontSize: 16 }}>SAHU CSC</span>
          </div>
          {customer && (
            <div style={{ background: "var(--brand-white-mid)", border: "1px solid var(--brand-white-25)", borderRadius: 18, padding: "16px 18px", marginBottom: 24, position: "relative" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 46, height: 46, borderRadius: 14, background: "var(--brand-white-border)", border: "2px solid var(--brand-white-30)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ...
          <div style={{ background: "var(--brand-white-mid)", border: "1px solid var(--brand-white-border)", borderRadius: 16, padding: "18px 20px", marginBottom: 12, position: "relative" }}>
          ...
          {entryAmt > 0 && (
            <div style={{ background: "var(--brand-white-low)", border: "1px solid var(--brand-white-high)", borderRadius: 14, padding: "14px 18px", marginTop: "auto", position: "relative" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--color-slate-50)" }}>
          <div style={{ background: "#fff", borderBottom: "1px solid var(--color-slate-100)", padding: "18px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--brand-navy-800)", margin: 0 }}>{isEdit ? t("udhari.customer.edit_entry") : isGave ? t("udhari.customer.you_gave") : t("udhari.customer.you_got")}</h2>
              <p style={{ fontSize: 12, color: "var(--color-slate-400)", margin: 0, marginTop: 2 }}>Udhari Khata › {customer?.name}</p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              {!isEdit && (
                <div style={{ display: "flex", background: "var(--color-slate-100)", borderRadius: 14, padding: 4, gap: 4 }}>
                  {(["gave", "got"] as const).map(typ => (
                    <button key={typ} type="button" onClick={() => setForm(p => ({ ...p, type: typ }))}
                      style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 11, border: "none", cursor: "pointer", background: form.type === typ ? (typ === "gave" ? "linear-gradient(135deg,var(--color-orange-900),var(--brand-orange-600))" : "linear-gradient(135deg,#064e3b,var(--color-success))") : "transparent", color: form.type === typ ? "#fff" : "var(--color-slate-500)", fontWeight: 700, fontSize: 13, boxShadow: form.type === typ ? (typ === "gave" ? "0 2px 10px rgba(234,88,12,0.35)" : "0 2px 10px color-mix(in srgb, var(--color-success) 35%, transparent)") : "none", transition: "all 0.15s" }}>
                      {typ === "gave" ? <><ArrowUpRight size={14} strokeWidth={2.5} /> {t("udhari.customer.you_gave")}</> : <><ArrowDownLeft size={14} strokeWidth={2.5} /> {t("udhari.customer.you_got")}</>}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ maxWidth: 560, display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              <div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))}
                    style={{ width: "100%", height: 50, paddingLeft: 40, paddingRight: 14, borderRadius: 14, border: "1.5px solid var(--color-slate-200)", fontSize: 14, fontWeight: 600, color: "var(--brand-navy-800)", outline: "none", background: "#fff", boxSizing: "border-box", boxShadow: "0 1px 4px var(--brand-navy-tint-md)" }}
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "var(--color-slate-600)", textTransform: "uppercase" as const, letterSpacing: "0.08em", display: "block", marginBottom: 8 }}>
                  Note <span style={{ fontWeight: 400, textTransform: "none" as const, color: "var(--color-slate-400)" }}>(optional)</span>
                </label>
                <div style={{ position: "relative" }}>
                  ...
                  <textarea value={form.note} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} rows={3}
                    ...
                    style={{ width: "100%", paddingLeft: 40, paddingRight: 14, paddingTop: 14, paddingBottom: 14, borderRadius: 14, border: "1.5px solid var(--color-slate-200)", fontSize: 14, color: "var(--brand-navy-800)", resize: "none", outline: "none", boxSizing: "border-box", fontFamily: "inherit", background: "#fff", lineHeight: 1.6, boxShadow: "0 1px 4px var(--brand-navy-tint-md)" }}
                    onFocus={e => (e.target.style.borderColor = accentColor)} onBlur={e => (e.target.style.borderColor = "var(--color-slate-200)")} />
                </div>
              </div>
            </div>
