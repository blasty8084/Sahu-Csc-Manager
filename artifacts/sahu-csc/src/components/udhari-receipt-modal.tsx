...
import type { UdhariReceiptModalProps } from "./udhari-receipt/udhariReceiptTypes";
import { UdhariReceiptCard } from "./udhari-receipt/UdhariReceiptCard";
import { useUdhariReceiptActions } from "./udhari-receipt/useUdhariReceiptActions";
...
export type { UdhariEntryReceipt } from "./udhari-receipt/udhariReceiptTypes";
...
export function UdhariReceiptModal({
  ...
  const year         = entry ? new Date(entry.date + "T00:00:00").getFullYear() : new Date().getFullYear();
  const receiptNumber = entry ? `UDH-${year}-${String(entry.id).padStart(4, "0")}` : "";
  ...
  const { handlePrint, handleDownloadPdf, handleWhatsApp, handleShare, generatingPdf, sendingWa } =
    useUdhariReceiptActions(entry, receiptNumber, verifyUrl, businessName, businessMobile, containerRef);
  ...
  const actions = [
    ...
    { icon: Share2,        label: "Share",                         onClick: handleShare,       disabled: false,        color: accentColor },
  ] as const;
