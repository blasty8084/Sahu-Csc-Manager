...
export function LedgerDesktopHeader({ t, onDeleteAll }: LedgerDesktopHeaderProps) {
  return (
    <div style={{ position: "relative", overflow: "hidden", background: "hsl(var(--card))", borderRadius: 20, flexShrink: 0, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid hsl(var(--border))" }}>
      {/* Gradient accent bar */}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 60, position: "relative", zIndex: 2 }}>
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: "hsl(var(--foreground))", margin: 0 }}>{t("ledger.title") || "Ledger"}</h1>
          <p style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", fontWeight: 500, marginTop: 2 }}>{t("ledger.subtitle") || "Track all your transactions and manage records seamlessly."}</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <a href="/api/reports/export" target="_blank"
            style={{ display: "flex", alignItems: "center", gap: 6, height: 34, paddingInline: 14, borderRadius: 10, border: "1.5px solid hsl(var(--border))", background: "hsl(var(--muted))", color: "hsl(var(--foreground))", fontSize: 12, fontWeight: 600, textDecoration: "none", cursor: "pointer" }}>
