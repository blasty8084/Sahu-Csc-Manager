...
import { useUpdateService } from "@workspace/api-client-react";
...
/**
 * Edit-service dialog: mobile uses a bottom sheet Dialog;
 * desktop uses a full-screen two-panel layout.
 */
export function ServiceEditDialog({ open, service, onClose, services, onSuccess }: ServiceEditDialogProps) {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      await updateMut.mutateAsync({ id: service.id, data: values });
        ...
        <DialogContent className="w-[calc(100vw-2rem)] max-w-md rounded-2xl md:rounded-lg">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <DialogFooter className="gap-2">
              ...
              <Button type="submit" disabled={updateMut.isPending} data-testid="button-save-service">Save</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex" }}>
        ...
        <div style={{ width: 380, flexShrink: 0, background: "linear-gradient(160deg,#7c2d12 0%,#c2410c 50%,#f97316 100%)", display: "flex", flexDirection: "column", padding: "40px 36px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", marginBottom: 28 }}>
            ...
            <h1 style={{ color: "#fff", fontSize: 24, fontWeight: 900, lineHeight: 1.2, marginBottom: 8 }}>Edit Service</h1>
