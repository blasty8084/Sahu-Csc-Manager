# SAHU CSC — Pre-v3 Feature Archive (v1.x & v2.x)

> **This file is the historical archive for pre-v3 features only.**
> For every v3.x change (current version), see `CHANGELOG_V3.md` — that is the canonical, actively-maintained changelog.
> For v2.x-era summaries, also see `docs/archive/changelogV2.md`.
>
> The sections below (1–36) document the original v1/v2 build: project foundation, auth, ledger, AePS, services, reports, notifications, audit logs, backups, PWA, TWA, push notifications, schema, and the June 2026 bug-fix/migration/UI-redesign work that preceded the V3 rewrite. Nothing here is duplicated in `CHANGELOG_V3.md`.
