...
| **Bundle optimizations** | 4 frontend improvements: (1) `index.html` OG/Twitter meta tags updated to `og-image.webp` (file already existed). (2) `Login` page made lazy (`React.lazy`) — moved ~54 kB out of the main bundle. (3) `reports.tsx` uses `React.lazy` + `Suspense` + `IntersectionObserver` for `MobileReports`/`DesktopReports` — `vendor-charts` (421 kB) now only loads when the Reports page is visited; page chunk shrank from 56 kB → 2.2 kB. (4) New `manualChunks` entries: `vendor-query-persist`, `vendor-idb` (dexie + idb-keyval), `vendor-pdf` (jspdf), `vendor-pdf-canvas` (html2canvas) — stable names survive hash renames and the SW `globIgnores` is updated to match. Main bundle: 284 kB → 232 kB (−52 kB, −18%). |
| **Workflow cleanup** | `Seed Database` now pushes schema before seeding (self-contained, safe to run standalone). `Worker Server` uses clean `exit 0` instead of printing a skip message. `Start application` removed — was a duplicate of `artifacts/sahu-csc: web` on port 5000 that always failed. `Project` runButton updated to launch `artifacts/sahu-csc: web` directly. |
