...
import { Mail, Smartphone, ChevronRight, Loader2, ArrowLeft } from "lucide-react";
...
interface Props {
  ...
  onChoose: (method: "otp" | "totp") => void;
  onBack: () => void;
}

/** Phase 1: two method-choice cards (Email OTP + Authenticator App) + back button. */
export function TwoFactorMethodPicker({ choosing, chooseError, maskedEmail, totpEnrolled, onChoose, onBack }: Props) {
  return (
      ...
      <button type="button" onClick={() => onChoose("totp")} disabled={!!choosing}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {totpEnrolled ? "Use your Google Authenticator, Authy, or TOTP app" : "Set up Google Authenticator, Authy, or any TOTP app"}
          </p>
        </div>
        <ChevronRight className="w-4 h-4 text-gray-400 flex-shrink-0" />
      </button>
