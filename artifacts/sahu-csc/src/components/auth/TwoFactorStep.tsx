...
import {
  ...
  Copy, Check, AlertTriangle, ChevronRight, QrCode, Eye, EyeOff,
} from "lucide-react";
...
export function TwoFactorStep({ challenge, onSuccess, onBack }: TwoFactorStepProps) {
  ...
  // ── TOTP enrollment QR data (mid-login first-time setup) ─────────────────────
  ...
  const handleChooseMethod = async (next: Method) => {
    ...
    try {
      ...
      if (next === "otp") {
        ...
        // Auto-enroll if not yet set up — server stores secret and returns QR data.
  ...
  // Determine if this is a first-time TOTP enrollment (we have QR data)
          ...
          <motion.div key={`code-entry-${method}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between text-xs">
                <button type="button"
                  onClick={() => { setMethod(null); resetCodeEntry(); if (resendTimerRef.current) clearInterval(resendTimerRef.current); setResendSeconds(0); setEnrollQrDataUrl(null); setEnrollSecret(null); }}
