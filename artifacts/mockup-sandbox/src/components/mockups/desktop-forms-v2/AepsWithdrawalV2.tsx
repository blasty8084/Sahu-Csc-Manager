import { ArrowDownLeft, IndianRupee, User, Fingerprint, Building2, FileText, Eye, Sparkles, Wallet, ShieldCheck, AlertCircle } from "lucide-react";
...
export function AepsWithdrawalV2() {
  return (
    <div style={{ width: "100vw", height: "100vh", display: "flex", fontFamily: "'Inter', system-ui, sans-serif", background: "#f8fafc", overflow: "hidden" }}>

      {/* ── LEFT PANEL ── */}
      <div style={{ width: 380, flexShrink: 0, background: "linear-gradient(160deg,#7f1d1d 0%,#9f1239 50%,#e11d48 100%)", display: "flex", flexDirection: "column", padding: "40px 36px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", marginBottom: 24 }}>
          <div style={{ width: 64, height: 64, borderRadius: 20, background: "rgba(255,255,255,0.15)", border: "2px solid rgba(255,255,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18 }}>
            <ArrowDownLeft size={30} color="#fff" strokeWidth={2.5} />
          </div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(255,255,255,0.14)", border: "1px solid rgba(255,255,255,0.22)", borderRadius: 8, padding: "4px 10px", marginBottom: 10 }}>
            <Sparkles size={10} color="#fca5a5" />
            <span style={{ color: "#fca5a5", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em" }}>AePS Withdrawal</span>
          </div>
          <h1 style={{ color: "#fff", fontSize: 26, fontWeight: 900, lineHeight: 1.2, marginBottom: 10 }}>Aadhaar-Based<br />Withdrawal</h1>
          <p style={{ color: "rgba(255,255,255,0.58)", fontSize: 13, lineHeight: 1.7 }}>Biometric cash withdrawal using Aadhaar authentication. Deducted from your AePS daily session.</p>
        </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.20)", borderRadius: 16, padding: "18px 20px", marginBottom: 16, position: "relative" }}>
          ...
          <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
            <div style={{ flex: 1, background: "rgba(255,255,255,0.10)", borderRadius: 8, padding: "8px 10px" }}>
              <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 9, fontWeight: 600, textTransform: "uppercase" }}>Withdrawals</p>
              <p style={{ color: "#fca5a5", fontSize: 13, fontWeight: 800, marginTop: 2 }}>₹5,000</p>
            </div>
            <div style={{ flex: 1, background: "rgba(255,255,255,0.10)", borderRadius: 8, padding: "8px 10px" }}>
              <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 9, fontWeight: 600, textTransform: "uppercase" }}>Remaining</p>
              <p style={{ color: "#fff", fontSize: 13, fontWeight: 800, marginTop: 2 }}>₹12,500</p>
            </div>
          </div>
        </div>

        {/* Security badge */}
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.14)", borderRadius: 12, padding: "12px 14px", position: "relative" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #f1f5f9", padding: "18px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: navy, margin: 0 }}>New Withdrawal</h2>
            <p style={{ fontSize: 12, color: "#94a3b8", margin: 0, marginTop: 2 }}>AePS · 20 Jun 2026 · Session active</p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(225,29,72,0.07)", border: "1px solid rgba(225,29,72,0.18)", borderRadius: 10, padding: "6px 12px" }}>
            <ArrowDownLeft size={13} color={accentColor} strokeWidth={2.5} />
            <span style={{ fontSize: 12, fontWeight: 700, color: accentColor }}>Withdrawal</span>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 40px" }}>
          <div style={{ maxWidth: 620, display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            {/* Amount — hero */}
            <div style={{ background: "linear-gradient(135deg,rgba(244,63,94,0.06),rgba(225,29,72,0.03))", border: "2px solid rgba(225,29,72,0.22)", borderRadius: 20, padding: "20px 24px" }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: accentColor, textTransform: "uppercase", letterSpacing: "0.1em", display: "block", marginBottom: 10 }}>Withdrawal Amount (₹) *</label>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 14 }}>
                <div style={{ width: 48, height: 48, borderRadius: 15, background: accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 4px 14px rgba(225,29,72,0.30)" }}>
                  <IndianRupee size={22} color="#fff" />
                </div>
                <input readOnly value="5,000.00" style={{ flex: 1, fontSize: 38, fontWeight: 900, color: accentColor, background: "transparent", border: "none", outline: "none", letterSpacing: "-0.02em" }} />
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {QUICK.map(v => (
                  <button key={v} style={{ padding: "7px 14px", borderRadius: 10, fontSize: 12, fontWeight: 700, background: v === 5000 ? accent : "#fff", color: v === 5000 ? "#fff" : "#64748b", border: `1px solid ${v === 5000 ? "transparent" : "#e2e8f0"}`, cursor: "pointer", boxShadow: v === 5000 ? "0 2px 8px rgba(225,29,72,0.25)" : "none" }}>
                    ₹{v >= 1000 ? v / 1000 + "K" : v}
                  </button>
                ))}
              </div>
              <p style={{ fontSize: 11, color: accentColor, marginTop: 10, fontWeight: 600 }}>After withdrawal: ₹12,500.00 remaining in session</p>
            </div>

            {/* Customer + Aadhaar grid */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              ...
              <div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input readOnly value="XXXX XXXX 4521" style={{ width: "100%", height: 50, paddingLeft: 40, paddingRight: 40, borderRadius: 14, border: `1.5px solid rgba(11,44,96,0.22)`, fontSize: 14, fontWeight: 700, color: navy, outline: "none", background: "rgba(11,44,96,0.02)", boxSizing: "border-box", fontFamily: "monospace", letterSpacing: "0.05em", boxShadow: "0 1px 4px rgba(11,44,96,0.06)" }} />
                  <Eye size={14} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", color: "#94a3b8", cursor: "pointer" }} />
                </div>
                <div style={{ display: "flex", gap: 2, marginTop: 6 }}>
                  {Array.from({ length: 12 }).map((_, i) => <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i < 12 ? navy : "#e2e8f0" }} />)}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 14, padding: "14px 18px" }}>
              ...
              <p style={{ fontSize: 12, color: "#9a3412", lineHeight: 1.6 }}>Confirm Aadhaar number and amount with the customer before proceeding. This transaction cannot be reversed.</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "#fff", borderTop: "1px solid #f1f5f9", padding: "20px 40px", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
          ...
          <button style={{ flex: 1, height: 50, borderRadius: 14, border: "none", cursor: "pointer", background: accent, color: "#fff", fontSize: 15, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: "0 6px 20px rgba(225,29,72,0.30)" }}>
            <ArrowDownLeft size={18} strokeWidth={2.5} /> Review Withdrawal — ₹5,000
          </button>
        </div>
      </div>
    </div>
